...
//		return result;  //用于测试最大值
    ...
    public String intToBinary(String number, int len)
    {
        return integerRepresentation(number,len);
    }

    public int[] StringToArray(String src, int len)
    {
        if(src.length() > len) {System.out.println("Long String Can't Transform To Short Array!");return null;}
        int[] res = new int[len];
        ...
        for(int i = 0 ; i < len; ++i)
        {
            if(str_len + i < len)
                ...
                res[i] = src.charAt(str_len + i - len) - '0';
        }
        return res;
    }

    public String ArrayToString(int[] src)
    {
    ...
    public String ArrayToString(char[] src)
    {
        StringBuffer res = new StringBuffer();
        for (char j : src)
            res.append(j);
    ...
    public char[] StringToCharArray(String src)
    {
        int len = src.length();
        char[] res = new char[len];
        for(int i = 0; i < len; ++i){
