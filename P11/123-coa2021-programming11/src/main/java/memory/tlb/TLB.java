...
public class TLB {
    ...
    public int isPageInTLB(int vPageNo)
    {
        for (int i = 0; i < TLB_SIZE; ++i)
        {
            // 老问题了，valid优先级最高
            // 不然可能出现 虚拟页号匹配，但其实此时这个页表项是无效的
            if(getTLBLine(i).valid && getTLBLine(i).vPageNo == vPageNo)
                return i;
        }
        return -1;
    }

    public char[] getPageFrame(int row)
    {
        return getTLBLine(row).pageFrame;
    }
